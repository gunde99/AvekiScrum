...
export function WorkItemTaskboardTab({ detail, onOpenRelation }: WorkItemTaskboardTabProps) {
  const tasks = detail.children.filter((c) => c.type === "Task");

  if (tasks.length === 0) {
    return <p className="wi-empty-state">Inga tasks under det här kortet.</p>;
  }
